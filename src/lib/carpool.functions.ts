...
async function createEventForUser(userId: string, input: EventInput) {
  const { generateShareCode } = await import("./session.server");
  const { supabaseAdmin } =
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    if (error?.code !== "23505")
      throw new Error("Could not create the carpool.");
  }
...
export const createEvent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return createEventForUser(user.id, data);
  });
...
export const createEventWithIdentity = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { createIdentitySession } = await import("./session.server");
    const user = await createIdentitySession(
      data.identity.username,
      data.identity.phone,
    );
    return createEventForUser(user.id, data.event);
  });
...
export const getEventPage = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<EventPage | null> => {
    ...
    const { data: event } = await supabaseAdmin
      .from("events")
      .select(
        "id, name, date, time, destination, share_code, created_by_user_id",
      )
    ...
    const { data: cars } = await supabaseAdmin
      .from("cars")
      .select(
        "id, driver_user_id, available_seats, pickup_location, departure_time, note, created_at, users!cars_driver_user_id_fkey(username), car_members(id, user_id, created_at, note, users(username))",
      )
...
export const addCar = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (error) {
      if (error.code === "23505")
        throw new Error("You already added a car to this carpool.");
...
export const updateCar = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (car.driver_user_id !== user.id)
      throw new Error("Only the driver can edit this car.");
    ...
    const { error } = await supabaseAdmin
      ...
      .update({
        available_seats: data.car.available_seats,
        pickup_location: data.car.pickup_location,
        departure_time: data.car.departure_time
          ? data.car.departure_time
          : null,
        note: data.car.note ? data.car.note : null,
      })
      .eq("id", car.id);
...
export const joinCar = createServerFn({ method: "POST" })
  .validator((data: unknown) => ({
    carId: String((data as { carId: string }).carId),
  }))
  .handler(async ({ data }) => {
    const { requireCurrentUser } = await import("./session.server");
    const { supabaseAdmin } =
    ...
    if (result === "full")
      throw new Error("Sorry — that was the last seat and it just went.");
    if (result === "is_driver")
      throw new Error("You're the driver of this car.");
    if (result === "not_found") throw new Error("This car no longer exists.");
    return { ok: true };
  });

export const addPassenger = createServerFn({ method: "POST" })
  .validator((data: unknown) => {
    const raw = data as { carId?: unknown; identity?: unknown };
    return {
      carId: String(raw.carId),
      identity: passengerSchema.parse(raw.identity),
    };
  })
  .handler(async ({ data }) => {
    const { requireCurrentUser } = await import("./session.server");
    ...
    if (
      car.driver_user_id !== actor.id &&
      event.created_by_user_id !== actor.id
    ) {
      throw new Error("Only the driver or organiser can add passengers.");
    }

    const { data: existingUsers, error: findError } = await supabaseAdmin
      .from("users")
      .select("id")
      .eq("phone_number", data.identity.phone)
      .order("created_at", { ascending: true })
      .limit(1);
    if (findError) throw new Error("Could not add this passenger.");

    let passengerId = existingUsers?.[0]?.id;
    ...
    if (!passengerId) {
      const { data: passenger, error } = await supabaseAdmin
        ...
        .select("id")
        .single();
      if (error || !passenger) throw new Error("Could not add this passenger.");
    ...
    const { data: result, error } = await supabaseAdmin.rpc("join_car", {
      _car_id: car.id,
      _user_id: passengerId,
    });

    if (error || result !== "ok") {
      if (createdPassenger) {
        await supabaseAdmin.from("users").delete().eq("id", passengerId);
      }
      if (result === "full") throw new Error("This car is already full.");
      if (result === "is_driver")
        throw new Error("The driver cannot also be a passenger.");
      if (result === "already_member")
        throw new Error("This passenger is already in the car.");
      throw new Error("Could not add this passenger.");
    }

    const { error: noteError } = await supabaseAdmin
      .from("car_members")
      .update({ note: data.identity.note || null })
      .eq("car_id", car.id)
      .eq("user_id", passengerId);
    if (noteError)
      throw new Error(
        "The passenger was added, but the note could not be saved.",
      );
...
export const removePassenger = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: member } = await supabaseAdmin
      .from("car_members")
      .select(
        "id, cars!inner(driver_user_id, events!inner(created_by_user_id))",
      )
      .eq("id", data.memberId)
      .maybeSingle();
    ...
    if (
      car.driver_user_id !== user.id &&
      car.events.created_by_user_id !== user.id
    ) {
